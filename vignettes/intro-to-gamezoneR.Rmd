...
```{r pbp}
pbp <- gamezoneR::gamezone_cbb_pbp(schedule$game_id[2],
                                   sub_parse = T)
...
duke_pbp <- purrr::map_df(duke_schedule$game_id, 
                          gamezoneR::gamezone_cbb_pbp, sub_parse = F)
```
...
Since `gamezoneR` version 0.1.1 now attempts to parse possession information from the play-by-play, we can also look at a team's offensive and defensive efficiency (by points per possession). Below is Duke's offensive and defensive efficiency by game from the 2018-19 college basektball season.

```{r duke_ppp}
duke_pbp %>%
  filter(!is.na(poss_before)) %>%
  ...
  group_by(date, game_id, poss_before, poss_number) %>%
  summarise(fgm = sum(shot_outcome == "made" & free_throw == F, na.rm = T),
            ...
            .groups = "drop") %>%
  group_by(date, game_id, poss_before) %>%
  summarise(poss = dplyr::n(),
            across(fgm:points, sum),
            .groups = "drop") %>%
  mutate(ppp = points/poss,
...
pbp_summarized <- pbp %>%
  ...
  dplyr::mutate(poss_number = as.numeric(poss_number),
                ...
                points = dplyr::case_when(
                  ...
                  shot_made_numeric == 1 & three_pt == 1 ~ 3,
                  shot_made_numeric == 1 & three_pt == 0 & free_throw == 0 ~ 2)) %>%
  group_by(season, date, game_id, poss_before, poss_number) %>%
  summarise(fgm = sum(shot_outcome == "made" & free_throw == F, na.rm = T),
            fga = sum(!is.na(shot_outcome) & free_throw == F),
            ftm = sum(shot_outcome == "made" & free_throw == T),
            fta = sum(!is.na(shot_outcome) & free_throw == T),
            points = sum(points, na.rm = T),
            .groups = "drop") %>%
  group_by(season, date, game_id, poss_before) %>%
  summarise(poss = dplyr::n(),
            across(fgm:points, sum),
            .groups = "drop") %>%
  mutate(ppp = points/poss)
...
Let's take a look at the most proficient offenses in college basketball since 2017-18

```{r gt_table}
pbp_summarized %>%
  group_by(season, poss_before) %>%
  summarise(games = n(),
  ...
  arrange(desc(ppp)) %>%
  head(15) %>%
  left_join(gamezoneR::cbb_team_info %>%
              select(game_zone, team_logo_espn),
            by = c("poss_before" = "game_zone")) %>%
  select(season, team_logo_espn, everything()) %>%
  gt::gt() %>%
  gt::tab_header(title = gt::md("**Best team offensive efficiencies**"),
                 subtitle = gt::md("*2017-18 through 2020-21 college basketball seasons*")) %>%
